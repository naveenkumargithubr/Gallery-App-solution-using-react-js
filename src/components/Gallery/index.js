...
class Gallery extends Component {
  state = {isImageSelected: imagesList[0].id}

  updateThumbId = id => {
    this.setState({isImageSelected: id})
  }

  render() {
    const {isImageSelected} = this.state
    const {imageUrl, imageAltText} = imagesList[isImageSelected]
            ...
            <ul className="unordered-list-container">
              {imagesList.map(eachItem => (
                <ThumbnailItem
                  ...
                  key={eachItem.id}
                  updateThumbId={this.updateThumbId}
                />
              ))}
            </ul>
          </div>
